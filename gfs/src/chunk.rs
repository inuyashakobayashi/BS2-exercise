...
impl ChunkServer {
	...
	async fn heartbeat_loop(self) {
		...
		loop {
			...
			// Collect held chunk IDs in a scope so the std Mutex guard is
			// dropped before the .await below; it is !Send across an await.
	...
	async fn data_loop(self, listener: TcpListener) {
		...
		// The data plane is raw TCP rather than tarpc: chunk payloads can be
		// large and are framed directly instead of JSON-RPC encoded.
		loop {
	...
	async fn handle_request(&self, req: DataRequest) -> DataResponse {
		match req {
			// forward_to: Some(addr) means we are the primary and must
			// replicate to the secondary before acking; None means we are the
			// secondary, so we store and ack without forwarding further.
			DataRequest::Write {
				chunk_id,
				forward_to,
				payload,
			...
			//  byte-range read returning only [offset, offset+len).
