...
struct ServerInfo {
	/// Data-plane TCP address handed to clients for chunk reads/writes.
	addr: SocketAddr,
	/// Timestamp of the last heartbeat; used for timeout detection.
	last_seen: Instant,
	online: bool,
}
...
struct ChunkInfo {
	/// Servers holding this chunk, in write order [primary, secondary].
	replicas: Vec<ServerId>,
	/// Marked when the file was deleted; awaits lazy GC via heartbeat.
	deleted: bool,
}
...
impl ClientMaster for MasterServer {
	...
	async fn allocate(
		...
		for _ in 0..n_chunks {
			...
			// Record the intended placement now; a later heartbeat that
			// reports this chunk is what confirms the write actually landed.
			state.chunks.insert(
				chunk_id,
				ChunkInfo {
					replicas,
...
impl ChunkMaster for MasterServer {
	...
	async fn heartbeat(
		...
		// The heartbeat is the only chunk-server -> master channel: the master
		// reconstructs all chunk state by diffing chunks_held against its own
		// metadata, so there are no separate "write done" / "delete done" RPCs.
